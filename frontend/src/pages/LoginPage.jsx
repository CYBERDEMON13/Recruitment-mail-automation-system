...
function ClerkGoogleSignInBtn({ onGoogleSuccess, fallbackGoogleSignIn, googleLoading, setGoogleLoading }) {
  let clerk = null;
  let clerkUser = null;
  ...
  return (
    <button type="button" className="google-btn" onClick={handleClick} disabled={googleLoading}>
      {googleLoading ? <span className="spinner" style={{ borderColor: '#3b82f6', borderTopColor: 'transparent' }} /> : (
        <>
          ...
          <span>{isClerkConfigured ? 'Continue with Google (Clerk Auth)' : 'Continue with Google'}</span>
        </>
      )}
    </button>
  );
...
export default function LoginPage() {
  ...
  const fallbackGoogleSignIn = async () => {
    ...
    await processGoogleAuthPayload({
      email: gmailInput,
      ...
      <div style={{
        ...
        <div style={{ width: '100%', maxWidth: '1000px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', alignItems: 'center' }}>
          ...
          <div style={{
            ...
            {/* Google Sign In */}
            <ClerkGoogleSignInBtn
              onGoogleSuccess={processGoogleAuthPayload}
              fallbackGoogleSignIn={fallbackGoogleSignIn}
